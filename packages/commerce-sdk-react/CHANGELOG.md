...
- [Feature] Enable `throwOnMaintenanceHeader: true` by default in the SDK client config so that `MaintenanceError` is thrown automatically when the `sfdc_maintenance` response header is detected.
