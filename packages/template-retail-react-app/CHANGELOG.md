## v10.1.0-dev
- [Bugfix] Fix PayPal/Venmo express checkout: resolve buyer-approved address from the upstream PayPal Order via `getBasket?expand=paymentreferences` (the SDK does not provide it client-side), and PATCH the basket payment instrument on shipping address change so the PayPal review screen reflects the new amount and shipping options. [#3868](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3868)
